...
const thiredEye = async (req, res) => {
    try {
       ...
       const { id } = req.params;
        ...
        const { name, address, cinNo, gstinNo, licenseNo, panNo, aadharNo, mobileNo } = req.body;
        let searchQuery = {};
        ...
        if (cinNo) searchQuery.cinNumber = cinNo;
        if (gstinNo) searchQuery.gstin = gstinNo;
        if (licenseNo) searchQuery.licenseNumber = licenseNo;
        if (panNo) searchQuery.panNumber = panNo; //not in db
        if (aadharNo) searchQuery.aadharCardNumber = aadharNo;
        if (mobileNo) searchQuery.mobileNumber = mobileNo;
        // leader id 
        console.log("search query is commijn",searchQuery);
        ...
        const usersdata = user.length > 0 ? user.map(team => team) : [];

        const companyId = id;

        let dealerRetailerDetails = await SalesInvoice.find({ 
            companyId,
            $or: [ searchQuery ]
         }).lean();
        console.log("Dealer/Retailer Details:", dealerRetailerDetails);
